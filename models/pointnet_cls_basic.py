...
def my_conv(points, normals, adjacent_id, is_training, KNN=16, unique=False, bn_decay=None):
	...
	distances, point_indices = tf.nn.top_k(-D, k=KNN, sorted=True)  # point_indices: bs * N * KNN
	print('point_indices shape ',np.shape(point_indices), '  ', point_indices)
	print('my adjacent id shape is ',np.shape(adjacent_id))
	batch_indices = tf.tile(tf.reshape(tf.range(batch_size), (-1, 1, 1, 1)), (1, point_num, KNN, 1))
	indices = tf.concat([batch_indices, tf.expand_dims(adjacent_id, axis=3)], axis=3)
	# --------------------------------------------------------------- #
	...
	fenzi = tf.reduce_sum(normal_of_knn_points * first_knn_points, axis=-1, keep_dims=True) # [batch_size, point_num, K, 1]
	...
	with tf.variable_scope('my_conv') as sc:
		...
		local_feature = tf_util.max_pool2d(local_feature, [1, KNN],stride = [1,1], padding='VALID', scope='local_maxpool') # [bs,1024,1,64]
...
def get_model(point_cloud, normals, is_training, adjacent_id, KNN, bn_decay=None, num_classes = 40):
	""" Classification PointNet, input is BxNx3, output B x num_classes """
	...
	local_feature, points_weight = my_conv(point_cloud, normals, adjacent_id, is_training=is_training, KNN=KNN, bn_decay=bn_decay)
	# ------------------------------------------------------ #
